...
public class RSAUtil {
    ...
    //公钥加密
    public static String publicEncrypt(String context,String publicKey) throws Exception{
        PublicKey key = string2PublicKey(publicKey);
        byte[] bytes = publicEncrypt(context.getBytes("UTF-8"),key);
        return byte2Base64(bytes);
    }
    ...
    public static void main(String[] args) throws Exception{
        String context = "高旭刚";
        String result = publicEncrypt(context,Constants.PUBLIC_KEY);
        System.out.println("加密后的数据：" + result);

        System.out.println("解密后的数据："+privateDecrypt(result,Constants.PRIVATE_KEY));
    }
